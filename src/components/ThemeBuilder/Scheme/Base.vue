  ...
  <v-container fluid pa-0>
    <v-layout wrap row ma-0>
      ...
      >
        ...
        <!-- </v-card> -->
        <v-avatar :class="[color['500'].classname, 'cursor-pointer']" size="40">
          <v-icon
            large
            v-if="
              hasActive &&
                active.classname.startsWith(`md-${i.split('-').join('-')}`)
            "
  ...
  computed: {
    ...
    hasActive() {
      if (typeof this.selected === "string") {
        ...
        return (
          has(this.selected, "classname") && this.selected.classname !== null
        );
      }
    }
  }
